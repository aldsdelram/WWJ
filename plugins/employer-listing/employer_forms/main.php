...
include( 'modals.php' ); 
	...
	function employer_regform_edit_func( $atts ) {
		$default_atts = array(
			'step' 	=> 1
		);
		...
		<div id="employer-reg-form" class="btsp-container-fluid white-bg-views">
			...
			<div class="employer-reg-form-content">
				...
				<form action="" method="POST" id="employer<?= $step ?>" class="testform" enctype="multipart/form-data">
					<div class="form-container">
						<?php if ( $step == 1 ): ?>
							<?= employer_reg_edit_step_1(); ?>
						<?php elseif ( $step == 2 ): ?>
 							<?= employer_reg_edit_step_2(); ?>
						<?php else: ?>
							<?= employer_reg_edit_step_3(); ?>
						<?php endif ?>
					</div> <!-- end of .form-container -->
				</form>
	...
	add_shortcode( 'employer-edit-reg-form', 'employer_regform_edit_func' );
	// [employer-reg-form step=""]
